...
class PerformanceTracker:
    ...
    def record_performance(
        ...
        query = """
            INSERT INTO shorts_performance
                (video_id, bno, views, likes, comments, shares, ctr, avg_view_duration, last_updated)
            VALUES
                (:video_id, :bno, :views, :likes, :comments, :shares, :ctr, :avg_view_duration, NOW())
    ...
    def get_performance_stats(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            with self.engine.connect() as conn:
                ...
                # 최고 성과 영상 (PostgreSQL: LIMIT 1)
                top_query = f"""
                            SELECT sp.video_id, sp.views, sp.likes, b.title
                            FROM shorts_performance sp
                                     JOIN AI_BOARD b ON sp.bno = b.bno
                            WHERE sp.created_date > NOW() - INTERVAL '{days} days'
                            ORDER BY sp.views DESC
                            LIMIT 1
                            """

                top_result = conn.execute(sqlalchemy.text(top_query)).fetchone()
    ...
    def analyze_video_type_performance(self) -> Dict[str, Dict[str, float]]:
        ...
        query = """
                ...
                WHERE sp.created_date > NOW() - INTERVAL '30 days'
